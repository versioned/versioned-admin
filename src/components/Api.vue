<template>
  <div>
      ...
      <ul>
        <li>Base URL: <pre>{{baseUrl}}</pre></li>
        <li>Space ID: <pre>{{spaceId}}</pre></li>
        <li><a :href="docsUrl" target="_blank">API Docs</a></li>
      </ul>
  ...
  methods: {
    ...
    async getExamples (models, dbStats) {
      ...
      if (exampleModel) {
        ...
        result.push({name: `List ${exampleModel.name} Data`, httpie: Api.httpie(api.listUrl())})

        const docs = (await Api.listRequest(api.listUrl({limit: 1}))).data
        const id = u.getIn(docs, '0.id')
        const getUrl = api.getUrl(id)
        result.push({name: `Get One ${exampleModel.name}`, httpie: Api.httpie(getUrl)})
      }
      return result
    }
  }
